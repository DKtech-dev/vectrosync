...
export function AuditLedgerView() {
  ...
  return (
    <div className="flex flex-col gap-3 font-sans">
      <div className="flex items-center justify-between pb-2 border-b border-slate-200 text-xs font-medium">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-slate-800 font-mono uppercase tracking-wide">
            SHA-256 Cryptographic Provenance Chain &mdash; Tamper-Evident Ledger
          </span>
      ...
      {auditData && (
        <div className={`p-3.5 rounded-lg border flex items-center justify-between shadow-xs ${
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-mono font-bold text-xs">
                {auditData.is_chain_valid
                  ? 'Cryptographic Ledger Intact & Verified'
                  : 'Tamper Detected in Hash Chain'}
              </div>
              <div className="text-[11px] text-slate-600 font-mono mt-0.5">
                {auditData.block_count} blocks verified &middot; Genesis Hash: 0000000000000000
              </div>
            </div>
          </div>

          <div className="font-mono text-xs font-semibold px-2.5 py-1 rounded-md bg-emerald-100 border border-emerald-300 text-emerald-800">
            0 Alterations
          </div>
        </div>
      )}
          ...
          {auditData && auditData.events && auditData.events.length > 0 ? (
            <table className="w-full text-left">
              <thead className="bg-slate-50 text-slate-600 text-[10px] uppercase font-semibold">
                <tr>
                  <th className="p-2 border-b border-slate-200"># Block</th>
                  <th className="p-2 border-b border-slate-200">Timestamp</th>
                  <th className="p-2 border-b border-slate-200">Event Type</th>
                  <th className="p-2 border-b border-slate-200">Tag</th>
                  <th className="p-2 border-b border-slate-200">SHA-256 Digest</th>
                </tr>
              </thead>
              <tbody>
